...
class ConservativeIngestionTests(unittest.TestCase):
    ...
    def test_classify_page_unknown_when_only_weak_text_markers(self) -> None:
    ...
    def test_extract_wod_block_flags_ambiguous_without_movements_or_measures(self) -> None:
        text = "Warm-up notes\nFor Time\nGo hard\nStay focused"
        block, ambiguous = extract_wod_block(text)
        self.assertIsNotNone(block)
        self.assertTrue(ambiguous)

    def test_parse_one_marks_needs_review_when_wod_block_ambiguous(self) -> None:
        row = {
            "wod_date": "2024-01-01",
